...
import com.crawljax.visual.DHashStateVertexImpl;
import com.crawljax.visual.StateVertexImplVisual;
...
//		System.out.println("SFG: " + sfg.toString());

		Set<String> hashes = new TreeSet<String>();
		
		for (StateVertex sv : sfg.getAllStates()) {
			if (sv instanceof StateVertexImplVisual) {
				StateVertexImplVisual svv = (StateVertexImplVisual) sv;
				System.out.println("State: " + svv.getName() + " hash: " + svv.getpHashVisual());
				hashes.add(svv.getpHashVisual());
