...
import pageobject.PageObject.ChangeType;
...
public class ObjectDiff {
	...
	/**
	 * Annotate each of the {@code PageObject}s with a change label.
	 * 
	 * Currently uses a naive method of matching objects based on their image
	 * (MD5) hash, size, and location. A better algorithm can certainly be
	 * developed which uses:
	 * 	(1) Perceptual hashing, for finding approximate image matches
	 * 	(2) Graph diff, for finding blocks of elements that have moved
	 * 	(3) Text diff, for identifying where text has changed
	 */
	public void diff() {
	...
	/**
	 * @return {@code true} iff the transformation contains one or more edit
	 * operations.
	 */
	public boolean hasChanges() {
		for(PageObject po : oldPage.getPageObjects())
			if(po.getChangeType() != ChangeType.UNCHANGED) return true;
	...
	private Mat annotatePage(Page page) {
		...
		/* Draw a bounding box around each page objects, coloured according to
		* the page object's change type. */
