...
public class ObjectDetection {
	...
	/**
	 * @param pageObjectFactory Factory which builds the desired {@code
	 * 			  PageObject} type.
	 * @param inputFile
	...
	/**
	 * Detect objects on the web page. The annotated image is available through
	 * {@code getAnnotated} , while the processed image is available through
	 * {@code getAdjusted}.
	 */
	public void detectObjects() {
		...
		/*
		 * Increase sensitivity to light by adjusting the gain.
		 * 
		 * This doesn't have an effect on object detection... it is purely to make the edges easier
		 * for us to see.
		 */
	...
	private Mat drawContours(Mat adjusted, Mat image) {
		...
		for (MatOfPoint contour : contours) {
			...
			if (box.size().area() > 200) {
				...
				PageObject pageObject = pageObjectFactory.makePageObject(objImg, box.x, box.y, box.width, box.height);
