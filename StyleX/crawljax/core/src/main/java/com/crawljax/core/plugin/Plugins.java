...
public class Plugins {
	...
	static final ImmutableSet<Class<? extends Plugin>> KNOWN_PLUGINS = ImmutableSet
	        .of(DomChangeNotifierPlugin.class, OnBrowserCreatedPlugin.class,
	                ...
	                PreCrawlingPlugin.class, OnEventFiredPlugin.class);
    ...
    public void runOnEventFiredPlugins(CrawlerContext context, Eventable eventable, boolean firingSuccessful) {
		LOGGER.debug("Running OnEventFirePlugins...");
		counters.get(OnEventFiredPlugin.class).inc();
		for (Plugin plugin : plugins.get(OnEventFiredPlugin.class)) {
			if (plugin instanceof OnEventFiredPlugin) {
				try {
					LOGGER.debug("Calling plugin {}", plugin);
					((OnEventFiredPlugin) plugin).eventFired(context, eventable, firingSuccessful);
				} catch (RuntimeException e) {
					reportFailingPlugin(plugin, e);
				}
			}
