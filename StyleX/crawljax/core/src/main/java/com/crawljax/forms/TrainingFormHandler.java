...
public class TrainingFormHandler extends FormHandler {
	...
	private static Set<Identification> visited = new HashSet<Identification>();
	...
	public synchronized void handleFormElements(List<FormInput> formInputs) {
		 ...
		 * request user input. */
		if (incomplete(formInputs)) pauseAndTrainModal(formInputs);
		...
		for (FormInput input : formInputs) {
			...
			if(!visited.contains(input.getIdentification())) {
				...
				if(!inputElement.getAttribute("type").equalsIgnoreCase("hidden")
						&& inputElement.isDisplayed()) return true;
	 ...
	 * 
	 * @param formInputs
	 *            The input elements in the current DOM.
	 */
	private void pauseAndTrainModal(List<FormInput> formInputs) {
		...
		js += " modal.setAttribute('style', 'display:none;position:fixed;z-index:1;left:0;top:0;width:100%;height:100%;overflow:auto;background-color:rgb(0,0,0);background-color:rgba(0,0,0,0.4)');";
		...
		for (FormInput input : formInputs) {
			...
			Set<InputValue> inputValues = new HashSet<InputValue>();
