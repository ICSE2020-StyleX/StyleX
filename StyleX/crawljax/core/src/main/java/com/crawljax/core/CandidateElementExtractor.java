...
public class CandidateElementExtractor {
	...
	private final ElementSorter elementSorter;
	...
	public CandidateElementExtractor(ExtractorManager checker, @Assisted EmbeddedBrowser browser,
		...
		elementSorter = rules.getElementSorter();
	}
	...
	public ImmutableList<CandidateElement> extract(StateVertex currentState)
		...
		if (null != elementSorter) {
			results = new LinkedList<>(elementSorter.sort(results));
		} else if (randomizeElementsOrder) {
			Collections.shuffle(results);
		}
		currentState.setElementsFound(results);
		long end = System.nanoTime();
		LOG.info("Found {} new candidate elements to analyze in {} ns!", results.size(), end - start);
	...
	private void addFramesCandidates(Document dom, List<CandidateElement> results,
		...
		for (int i = 0; i < frameNodes.getLength(); i++) {
			...
			if (!Strings.isNullOrEmpty(relatedFrame)) {
				frameIdentification += relatedFrame + "<ISSTA>";
			}
...
//			if (xPathExpression.contains("/HEAD[1]/") || xPathExpression.contains("/SCRIPT[")) {
//				LOG.warn("Ignoring {}", xPathExpression);
//				continue;
//			}

			boolean satisfiesElementCrawlConditions = eventableConditionChecker.checkElementsConditions(browser, element);
			/*
			...
			if (matchesXpath && !checkedElements.isChecked(id)
					...
					&& satisfiesElementCrawlConditions) {
	...
	private void evaluateElement(List<CandidateElement> results, String relatedFrame,
		...
		if (eventableCondition != null && eventableCondition.getLinkedInputFields() != null
            ...
            // If the crawl element has an ElementCrawlEventsProvider, use it,
            // otherwise get the explicitly-provided events from the configs
			List<Eventable.EventType> eventTypes;
			if (crawl.hasEventsProvider()) {
				eventTypes = crawl.getEventsProvider().getEventTypes(browser, sourceElement);
			} else {
				eventTypes = crawl.getEventTypes();
			}
			candidateElements.add(new CandidateElement(sourceElement, new Identification(
			        Identification.How.xpath, xpath), relatedFrame, eventTypes));
		}
