...
public class EventableCondition {
	...
	private List<FormInput> linkedInputFields = new ArrayList<FormInput>();
	private List<ElementCondition> elementConditions = new ArrayList<>();
	...
	public void addElementConditions(ElementCondition... elementConditions) {
		this.elementConditions.addAll(Arrays.asList(elementConditions));
	}

	public boolean checkAllElementConditionsSatisfied(EmbeddedBrowser browser, Element element) {
		for (ElementCondition condition : elementConditions) {
			if (!condition.check(browser, element)) {
				return false;
			}
		}
		return true;
