...
public class Crawler {
	...
	@Inject
	Crawler(CrawlerContext context, CrawljaxConfiguration config, StateComparator stateComparator,
	...
	private boolean fireEvent(Eventable eventable) {
		...
		try {
			...
			if (crawlRules.isCrawlHiddenAnchors() && eventToFire.getElement() != null
				...
				LOG.info("Ignoring invisible element {}", eventToFire.getElement());
			}
			if (e instanceof NoSuchElementException) {
				LOG.warn("No such element {}", eventable.getIdentification().getValue());
			}
		...
		plugins.runOnEventFiredPlugins(context, eventable, isFired);
		if (isFired) {
