...
public final class CrawlElement {
	...
	private final List<ElementCondition> elementConditions = Lists.newLinkedList();
	private ElementCrawlEventsProvider eventsProvider;
	 ...
	 */
	protected CrawlElement(List<EventType> eventTypes, String tagName) {
	...
	protected CrawlElement(EventType eventType, String tagName) {
		this(Lists.newArrayList(eventType), tagName);
	}
	 ...
	 * withAtttribute("class", "foo") Would restrict this CrawlElement to only include the div with
	...
	/**
	 * @return The current {@link ElementCondition} objects associated with this crawl element.
	 */
	public List<ElementCondition> getElementConditions() {
		return this.elementConditions;
	}

	/**
	 * Add ElementConditions for a given element. For each candidate element,
	 * if the {@link ElementCondition#check(EmbeddedBrowser, Element)} function returns <code>true</code>,
	 * the element is considered for exploration.
	 * @param conditions An array of {@link ElementCondition} objects
	 * @return The current {@link CrawlElement} object to chain other calls
	 */
	public CrawlElement withElementConditions(ElementCondition... conditions) {
		this.elementConditions.addAll(Arrays.asList(conditions));
		return this;
	}

	public CrawlElement withEventsProvider(ElementCrawlEventsProvider eventsProvider) {
		this.eventsProvider = eventsProvider;
		return this;
	}

	public boolean hasEventsProvider() {
		return this.eventsProvider != null;
	}

	public ElementCrawlEventsProvider getEventsProvider() {
	...
	public EventableCondition getEventableCondition() {
		if ((getWithXpathExpression() == null || getWithXpathExpression().equals(""))
		        && getConditions().isEmpty() && getInputFieldIds().isEmpty() && getElementConditions().isEmpty()) {
			return null;
		}
		...
		if (getElementConditions().size() > 0) {
			eventableCondition.addElementConditions(getElementConditions().toArray(new ElementCondition[]{}));
		}
	...
	public String toString() {
		return MoreObjects.toStringHelper(this).add("tagName", tagName).add("conditions", conditions)
		        .add("id", id).add("eventType", eventTypes).add("inputFieldIds", inputFieldIds)
