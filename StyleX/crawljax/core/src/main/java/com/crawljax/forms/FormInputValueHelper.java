...
public final class FormInputValueHelper {
	...
	/**
	 * Removes the instance so another must be built. 
	 * 
	 * Used only to reset the state in between JUnit tests.
	 */
	...
	/**
	 * Get the form input for the DOM element. Not 
	 ...
	 * 
	 * @param dir
	 *            The output directory for the form input data.
	 * @throws Exception 
	 */
	public static void serializeFormInputs(File dir) {
		...
		try {
			...
			FileUtils.writeStringToFile(out, serialized);
		} catch (IOException e) {
			LOGGER.error(e.getMessage(), e);
		}
	...
	public static List<FormInput> deserializeFormInputs(File dir) {
		...
		List<FormInput> deserialized = new ArrayList<FormInput>();
		...
		if (in.exists()) {
			...
			try {
				deserialized = gson.fromJson(FileUtils.readFileToString(in),
