...
public class InMemoryStateFlowGraph implements Serializable, StateFlowGraph {
	 ...
	 * such vertex, the call leaves this graph unchanged and returns false. In
	 * combination with the restriction on constructors, this ensures that graphs
	 * never contain duplicate vertices. Throws java.lang.NullPointerException - if
	 * the specified vertex is null. This method automatically updates the state
	 * name to reflect the internal state counter.
	 * 
	 * @param stateVertix
	 ...
	 * combination with the restriction on constructors, this ensures that graphs
	 * never contain duplicate vertices. Throws java.lang.NullPointerException - if
	 * the specified vertex is null.
	 * 
	 * @param stateVertix
	 *            the state to be added.
	 * @param correctName
	 *            if true the name of the state will be corrected according to the
	...
	private StateVertex putIfAbsent(StateVertex stateVertix, boolean correctName) {
		...
		try {
			...
			if (added) {
				...
				LOG.debug("Graph already containes vertex {}", stateVertix);
	 ...
	 * target vertex. More formally, adds the specified edge, e, to this graph if
	 * this graph contains no edge e2 such that e2.equals(e). If this graph already
	 * contains such an edge, the call leaves this graph unchanged and returns
	 * false. Some graphs do not allow edge-multiplicity. In such cases, if the
	 * graph already contains an edge from the specified source to the specified
	 * target, than this method does not change the graph and returns false. If the
	 * edge was added to the graph, returns true. The source and target vertices
	 * must already be contained in this graph. If they are not found in graph
	...
	StateVertex newStateFor(String url, String dom, String strippedDom, EmbeddedBrowser browser) {
		...
		return vertexFactory.newStateVertex(id, url, getNewStateName(id), dom, strippedDom, browser);
	}
	...
	public List<List<GraphPath<StateVertex, Eventable>>> getAllPossiblePaths(StateVertex index) {
		...
		final KShortestPaths<StateVertex, Eventable> kPaths = new KShortestPaths<>(this.sfg, index, Integer.MAX_VALUE);
