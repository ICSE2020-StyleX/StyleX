...
public class CrawlActionsBuilder {
	...
	public static class ExcludeByParentBuilder {
		...
		private ImmutableList<CrawlElement> asExcludeList(
			...
			for (CrawlElement include : includes) {
				builder.add(new CrawlElement(include.getEventTypes(), include.getTagName()).underXPath(xpath));
			}
	...
	/**
	 * We don't know yet what to do with those elements, must add events later
	 * @param tagName
	 * @return
	 */
	public CrawlElement consider(String tagName) {
		checkNotRead();
		Preconditions.checkNotNull(tagName, "Tagname cannot be null");
		CrawlElement crawlTag = new CrawlElement(Lists.newArrayList(), tagName.toUpperCase());
		crawlElements.add(crawlTag);
		return crawlTag;
	}
