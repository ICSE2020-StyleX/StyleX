...
public final class WebDriverBackedEmbeddedBrowser implements EmbeddedBrowser {
	...
	public static WebDriverBackedEmbeddedBrowser withRemoteDriver(String hubUrl,
		...
		return WebDriverBackedEmbeddedBrowser.withDriver(buildRemoteWebDriver(hubUrl), filterAttributes, crawlWaitEvent,
				crawlWaitReload);
	}
	...
	/**
	 * Private used static method for creation of a RemoteWebDriver. Taking care of
	 * the default Capabilities and using the HttpCommandExecutor.
	 * 
	 * @param hubUrl
	...
	private static RemoteWebDriver buildRemoteWebDriver(String hubUrl) {
		...
		try {
			...
			LOGGER.error("Received unknown exception while creating the " + "HttpCommandExecutor, can not continue!",
					e);
			return null;
		}
	 ...
	 *            the period to wait after an event is fired.
	 */
	private WebDriverBackedEmbeddedBrowser(WebDriver driver, ImmutableSortedSet<String> filterAttributes,
	...
	private void handlePopups() {
		try {
			executeJavaScript("window.alert = function(msg){return true;};"
					+ "window.confirm = function(msg){return true;};" + "window.prompt = function(msg){return true;};");
	...
	private boolean fireEventWait(WebElement webElement, Eventable eventable)
		...
		case MOUSE_OUT:
		case MOUSE_OVER:
			fireMouseEvent(eventable);
			break;
		case TOUCHSTART:
		case TOUCHEND:
			fireTouchEvent(eventable);
			break;
		case CHANGE:
			fireChangeEvent(eventable);
			break;
		case DRAGSTART:
			fireDragEvent(eventable);
			break;
		case KEYDOWN:
			fireKeyEvent(eventable);
			break;
		default:
			LOGGER.warn("EventType {} is not supported", eventable.getEventType());
			return false;
		}
	...
	private boolean fireKeyEvent(Eventable eventable) {
		String keyEventJS =
				"var eventName = \"%s\";\n" +
		...
		executeJavaScript(String.format(keyEventJS, eventable.getEventType().getJsEventName(), xPathExpression));

		return true;
	}

	private boolean fireDragEvent(Eventable eventable) {
		String dragEventJS =
				"var eventName = \"%s\";\n" +
				"var element = document.evaluate(\"%s\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; \n" +
				...
				"}";
		String xPathExpression = eventable.getIdentification().getValue();
		executeJavaScript(String.format(dragEventJS, eventable.getEventType().getJsEventName(), xPathExpression));

		return true;
	}

	private boolean fireChangeEvent(Eventable eventable) {
		String changeJS =
				"var element = document.evaluate(\"%s\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; \n" +
				"if (element) {\n" +
				...
				"}";
		String xPathExpression = eventable.getIdentification().getValue();
		executeJavaScript(String.format(changeJS, xPathExpression));

		return true;
	}

	private boolean fireTouchEvent(Eventable eventable) {
		String touchJS =
				"var event = '%s'; \n" +
				"var element = document.evaluate(\"%s\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; \n" +
				"try { \n" +
				"  e = document.createEvent('TouchEvent'); \n" +
				"  e.initTouchEvent(event, true, true); \n" +
				"} catch (err) { \n" +
				"  try { \n" +
				...
				"}";
		String xPathExpression = eventable.getIdentification().getValue();
		executeJavaScript(String.format(touchJS, eventable.getEventType().getJsEventName(), xPathExpression));

		return true;
	}

	private boolean fireMouseEvent(Eventable eventable) {
		String mouseJS =
				...
				"}";
		String xPathExpression = eventable.getIdentification().getValue();
		executeJavaScript(String.format(mouseJS, xPathExpression, eventable.getEventType().getJsEventName()));
	...
	public synchronized boolean fireEventAndWait(Eventable eventable)
		...
		try {
			...
			WebElement webElement = browser.findElement(eventable.getIdentification().getWebDriverBy());
	...
	private void locateFrameAndgetSource(Document document, String topFrame, Element frameElement)
		...
		if (topFrame != null && !topFrame.equals("")) {
			frameIdentification += topFrame + "<ISSTA>";
		}
		...
		if (nameId != null && !ignoreFrameChecker.isFrameIgnored(frameIdentification + nameId)) {
			...
			try {
				...
				LOGGER.info("Invalid frame selector: " + frameIdentification + ", continuing...", e);
				browser.switchTo().defaultContent();
				return;
			}
			...
			try {
				...
				LOGGER.info("Got exception while inspecting a frame:" + frameIdentification + " continuing...", e);
			}
		}
	}
	...
	private void switchToFrame(String frameIdentification) throws NoSuchFrameException {
		...
		if (frameIdentification.contains("<ISSTA>")) {
			String[] frames = frameIdentification.split("<ISSTA>");
	 ...
	 * @return FormInput with random value assigned if possible. If no values were
	...
	private void setRandomValues(FormInput input, WebElement webElement, Set<InputValue> values) {
		...
		case TEXT:
		case TEXTAREA:
			values.add(new InputValue(new RandomInputValueGenerator().getRandomString(FormHandler.RANDOM_STRING_LENGTH),
					true));
			break;
		case CHECKBOX:
		case RADIO:
		...
		/**
		 * if (type.equals(InputType.TEXT) || type.equals(InputType.TEXTAREA)) {
		 * values.add(new InputValue(new RandomInputValueGenerator()
		 * .getRandomString(FormHandler.RANDOM_STRING_LENGTH), true)); } else if
		 * (type.equals("checkbox") || type.equals("radio") && !webElement.isSelected())
		 * { if (new RandomInputValueGenerator().getCheck()) { values.add(new
		 * InputValue("1", true)); } else { values.add(new InputValue("0", false)); } }
		 * else if (type.equals("select")) { Select select = new Select(webElement); if
		 * (!select.getOptions().isEmpty()) { WebElement option = new
		 * RandomInputValueGenerator().getRandomItem(select.getOptions());
		 * values.add(new InputValue(option.getText(), true)); }
		 * 
		 * }
		 **/
		...
		try {
			if (this.pixelDensity != -1) {
				...
				BufferedImage img = Shutterbug
						...
						.getImage();
				int newWidth = (int) (img.getWidth() / this.pixelDensity);
				int newHeight = (int) (img.getHeight() / this.pixelDensity);
			...
			final ShootingStrategy pasting = new ViewportPastingDecorator(new SimpleShootingStrategy())
	...
	private byte[] getScreenShotOriginal() throws CrawljaxException {
		try {
