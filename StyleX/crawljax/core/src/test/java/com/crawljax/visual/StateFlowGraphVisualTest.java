...
public class StateFlowGraphVisualTest {
	...
	@Before
	public void setup() {
		index = new StateVertexImplVisual(StateVertex.INDEX_ID, "index",
		        "<table><div>index</div></table>", "001");
		state2 = new StateVertexImplVisual(2, "STATE_TWO", "<table><div>state2</div></table>",
		        "002");
		state3 = new StateVertexImplVisual(3, "STATE_THREE", "<table><div>state3</div></table>",
		        "003");
		state4 = new StateVertexImplVisual(4, "STATE_FOUR", "<table><div>state4</div></table>",
		        "004");
		state5 = new StateVertexImplVisual(5, "STATE_FIVE", "<table><div>state5</div></table>",
		        "005");
		graph = new InMemoryStateFlowGraph(new ExitNotifier(0), new PHashStateVertexFactory());
	...
	public void testDuplicationAdding() throws Exception {
		...
		// should not be able to add a new state with the same phash value (should return a non-null
		// state equivalent to this one
		StateVertex stateReturned = graph.putIfAbsent(new StateVertexImplVisual(12, "STATE_12",
		        "<table><div><snan>state22</span></div></table>", "002"));

		assertThat(stateReturned, is(not(nullValue())));

		assertThat(((StateVertexImplVisual) stateReturned).getpHashVisual(), is("002"));
		...
		assertTrue(state2.equals(new StateVertexImplVisual(2, "STATE_2",
		        "<table><div>state2</div></table>", "002")));
		...
		StateVertex hold = new StateVertexImplVisual(StateVertex.INDEX_ID, index.getName(),
		        index.getDom(), "001");
	...
	public void testCloneStates() throws Exception {
		StateVertex state3clone2 = new StateVertexImplVisual(3, "STATE_THREE",
		        "<table><div>state2</div></table>", "002");
